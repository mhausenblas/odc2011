...
foreach ($councils as $council_id => $council) {
  ...
  $query = "SELECT * from applications WHERE council_id = $council_id ORDER BY received_date DESC LIMIT 0,10";
...
if (!mysql_query($query)) {
  echo "= DB UDPATE ERROR =".PHP_EOL.$query.PHP_EOL.mysql_error().PHP_EOL;
}